...
#-*- coding:UTF-8 -*-
                                               ...
                                               # 3. 制作模型训练的数据集比例：60%:20%:20%, 并制作txt文件为模型训练.
import os
import shutil
...
Root = 'V:\\Coding\\Thyroid_9long\\2ndTraining'
...
val_ratio = float(0.2)
test_ratio = float(0.2)    #这里的test集比例只是为了显示出, 数据集各个比例的划分, 实际并不参与运算就可以通过函数求出.#
name_of_class = []
...
class Build_for_training:
    def __init__(self, dataset_path, list_class, ratio_train, ratio_val, folder_type):
...
#                        position_dataset =range(nums - 1)
                        ...
                        train_list = random.sample( position_dataset, math.ceil(nums_train) )
                        print('number of train_index: %.2f files ~= %d files' % ( nums_train, math.ceil(nums_train) ) )
                        ...
                        #制作用于val的数据
                        margin_index = all_index - train_index
                        margin_nums = len(margin_index)
                        nums_val = nums * self.ratio_val
                        val_list = random.sample(margin_index, math.ceil(nums_val))
                        val_index = set(val_list)
                        print('number of val_index: %.2f files ~= %d files' % ( nums_val, math.ceil(nums_val) ) )
                        ...
                        #制作用于test的数据
                        test_index = all_index - train_index - val_index
                        print('number of test_index: %d files' % len(test_index) )
                        test_txt = self.dataset_path + '\\' + name + '\\' + 'test.txt'
...
def main():
    ...
    #建立一个文件夹放置可训练的数据集.
    work_3 = Build_for_training(training_folder, name_of_class, training_ratio, val_ratio, Type_Img)
    work_3.Creating()
